...
function listActionItems(options = {}) {
  ...
  if (!allActions || !allActions.length) {
    ...
    const actionsForToday = allActions
      .filter((action) => {
    ...
    console.log(
      ...
      chalk.gray.bold(`(Total: ${actionsForToday.length}): ✅ COMPLETE | ❌ NOT COMPLETE \n`)
    );
    ...
    const actionsFromYesterday = allActions
      .filter((action) => {
        return (new Date(action.createdAt)).toDateString() === getYesterdayDate().toDateString()
      });
    console.log(
      chalk.gray.bold(`List of all your actions from`),
      chalk.green.underline('YESTERDAY'),
      chalk.gray.bold(`(Total: ${actionsFromYesterday.length}): ✅ COMPLETE | ❌ NOT COMPLETE \n`)
    );
    actionsFromYesterday
    ...
    allActions.forEach((action = {}, index) => (
      action.complete
        ? console.log(
          chalk.green(`✅ (${index + 1}) ${action.text}`)
        )
        : console.log(
          chalk.gray(`❌ (${index + 1}) ${action.text}`)
        )
    ))
  }
}
